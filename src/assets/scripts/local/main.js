...
$(function() {
  if (window.mobileAndTabletcheck()) {
    $('html').addClass('non-parallaxable');
  } else {
    $('html').addClass('parallaxable');
  }
  ...
  $('.video-mobile').on('click', function(event) {
    ...
    $(this).siblings('.timecode').remove();

    $(this).find('iframe').attr('src', src).width(width).height(height);
  });

});

function securityAnimationInit() {
  var tl = new TimelineMax({
    repeat: -1,
    repeatDelay: 1.0
  });
  return; // leaving animation as static
  // var animationContainer = $('#security-animation');
  // animationContainer.find('> div').each(function() {
  //   TweenLite.to($(this), 0, {
  //     autoAlpha: 0
  //   });
  //   var transTime = 0.60;
  //
  //   tl.add(new TweenLite.to($(this), transTime, {
  //     autoAlpha: 1
  //   }));
  // });
}
...
function adjustParallaxElements() {
  'use strict';
  $('.parallaxable .parallax-tracking').each(function() {
    ...
    if ($(this).parent().index() > 0) { // these are fixed elements, that need to be aligned to their prev sibling's bottoms
      ...
      if (prevSibling.css('position') !== 'fixed') {
    ...
    if ($(window).width() < 800) { // no parallax in mobile breakpoint
...
function stickySection() {
  'use strict';
  if (windowLoaded) {
    $('#section-title').toggleClass('fixed', (latestKnownScrollY >= $('#header').height()));
    $('#copy').toggleClass('fixedSectionTitle', (latestKnownScrollY >= $('#header').height()));
  }
}
